...
type AwsApiGatewayStageLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AwsApiGatewayStage, err error)
...
// List lists all AwsApiGatewayStages in the indexer.
func (s *awsApiGatewayStageLister) List(selector labels.Selector) (ret []*v1.AwsApiGatewayStage, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AwsApiGatewayStage))
	})
	return ret, err
}
...
type AwsApiGatewayStageNamespaceLister interface {
	...
	Get(name string) (*v1.AwsApiGatewayStage, error)
...
// List lists all AwsApiGatewayStages in the indexer for a given namespace.
func (s awsApiGatewayStageNamespaceLister) List(selector labels.Selector) (ret []*v1.AwsApiGatewayStage, err error) {
...
// Get retrieves the AwsApiGatewayStage from the indexer for a given namespace and name.
func (s awsApiGatewayStageNamespaceLister) Get(name string) (*v1.AwsApiGatewayStage, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("awsapigatewaystage"), name)
	}
	return obj.(*v1.AwsApiGatewayStage), nil
}
