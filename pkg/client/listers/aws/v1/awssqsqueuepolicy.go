...
type AwsSqsQueuePolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AwsSqsQueuePolicy, err error)
...
// List lists all AwsSqsQueuePolicies in the indexer.
func (s *awsSqsQueuePolicyLister) List(selector labels.Selector) (ret []*v1.AwsSqsQueuePolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AwsSqsQueuePolicy))
	})
	return ret, err
}
...
type AwsSqsQueuePolicyNamespaceLister interface {
	...
	Get(name string) (*v1.AwsSqsQueuePolicy, error)
...
// List lists all AwsSqsQueuePolicies in the indexer for a given namespace.
func (s awsSqsQueuePolicyNamespaceLister) List(selector labels.Selector) (ret []*v1.AwsSqsQueuePolicy, err error) {
...
// Get retrieves the AwsSqsQueuePolicy from the indexer for a given namespace and name.
func (s awsSqsQueuePolicyNamespaceLister) Get(name string) (*v1.AwsSqsQueuePolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("awssqsqueuepolicy"), name)
	}
	return obj.(*v1.AwsSqsQueuePolicy), nil
}
