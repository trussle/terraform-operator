...
type AwsSesActiveReceiptRuleSetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AwsSesActiveReceiptRuleSet, err error)
...
// List lists all AwsSesActiveReceiptRuleSets in the indexer.
func (s *awsSesActiveReceiptRuleSetLister) List(selector labels.Selector) (ret []*v1.AwsSesActiveReceiptRuleSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AwsSesActiveReceiptRuleSet))
	})
	return ret, err
}
...
type AwsSesActiveReceiptRuleSetNamespaceLister interface {
	...
	Get(name string) (*v1.AwsSesActiveReceiptRuleSet, error)
...
// List lists all AwsSesActiveReceiptRuleSets in the indexer for a given namespace.
func (s awsSesActiveReceiptRuleSetNamespaceLister) List(selector labels.Selector) (ret []*v1.AwsSesActiveReceiptRuleSet, err error) {
...
// Get retrieves the AwsSesActiveReceiptRuleSet from the indexer for a given namespace and name.
func (s awsSesActiveReceiptRuleSetNamespaceLister) Get(name string) (*v1.AwsSesActiveReceiptRuleSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("awssesactivereceiptruleset"), name)
	}
	return obj.(*v1.AwsSesActiveReceiptRuleSet), nil
}
