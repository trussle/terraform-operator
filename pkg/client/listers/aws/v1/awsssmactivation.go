...
type AwsSsmActivationLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AwsSsmActivation, err error)
...
// List lists all AwsSsmActivations in the indexer.
func (s *awsSsmActivationLister) List(selector labels.Selector) (ret []*v1.AwsSsmActivation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AwsSsmActivation))
	})
	return ret, err
}
...
type AwsSsmActivationNamespaceLister interface {
	...
	Get(name string) (*v1.AwsSsmActivation, error)
...
// List lists all AwsSsmActivations in the indexer for a given namespace.
func (s awsSsmActivationNamespaceLister) List(selector labels.Selector) (ret []*v1.AwsSsmActivation, err error) {
...
// Get retrieves the AwsSsmActivation from the indexer for a given namespace and name.
func (s awsSsmActivationNamespaceLister) Get(name string) (*v1.AwsSsmActivation, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("awsssmactivation"), name)
	}
	return obj.(*v1.AwsSsmActivation), nil
}
