...
type AwsSubnetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AwsSubnet, err error)
...
// List lists all AwsSubnets in the indexer.
func (s *awsSubnetLister) List(selector labels.Selector) (ret []*v1.AwsSubnet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AwsSubnet))
	})
	return ret, err
}
...
type AwsSubnetNamespaceLister interface {
	...
	Get(name string) (*v1.AwsSubnet, error)
...
// List lists all AwsSubnets in the indexer for a given namespace.
func (s awsSubnetNamespaceLister) List(selector labels.Selector) (ret []*v1.AwsSubnet, err error) {
...
// Get retrieves the AwsSubnet from the indexer for a given namespace and name.
func (s awsSubnetNamespaceLister) Get(name string) (*v1.AwsSubnet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("awssubnet"), name)
	}
	return obj.(*v1.AwsSubnet), nil
}
